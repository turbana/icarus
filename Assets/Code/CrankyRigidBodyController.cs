                ...
                if (!Mathf.Approximately(inputX + inputY, 0.0f))
                {
                    ...
                    if (length > 0.0f)
                    {
                        if (length > MovementSpeed)
                        {
                            ...
                            Vector3 rbDirection = new Vector3(rb.velocity.x * length, 0.0f, rb.velocity.z * length);
     
                            // dot product of accel unit vector and velocity unit vector, clamped above 0 and inverted (1-x)
                            length = (1.0f - Mathf.Max(movement.x * rbDirection.x + movement.z * rbDirection.z, 0.0f)) * AirborneAccel * Time.deltaTime;
                            movement.x *= length;
                            movement.z *= length;
                        }
